...
module IonoscloudCertManager
  class CertificatePostPropertiesDto
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `IonoscloudCertManager::CertificatePostPropertiesDto` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `IonoscloudCertManager::CertificatePostPropertiesDto`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
    ...
    def list_invalid_properties
      ...
      if @certificate_chain.nil?
        invalid_properties.push('invalid value for "certificate_chain", certificate_chain cannot be nil.')
      end
    ...
    def valid?
      ...
      return false if @certificate_chain.nil?
    ...
    def hash
      [name, certificate, certificate_chain, private_key].hash
    end
